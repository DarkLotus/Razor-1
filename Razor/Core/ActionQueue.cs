	...
	public class DragDropManager
	{
		...
		private static void DropCurrent()
		{
			...
			if ( m_Holding.IsItem )
			{
				if ( World.Player.Backpack != null )
					ClientCommunication.Instance.SendToServer( new DropRequest( m_Holding, Point3D.MinusOne, World.Player.Backpack.Serial ) ); 
				else
					ClientCommunication.Instance.SendToServer( new DropRequest( m_Holding, World.Player.Position, Serial.Zero ) ); 
			}
			else
			{
		...
		public static int Drag( Item i, int amount, bool fromClient, bool doLast )
		{
			...
			if ( Full )
			{
				...
				if ( fromClient )
					ClientCommunication.Instance.SendToClient( new LiftRej() );
				return 0;
			}
		...
		public static bool Drop( Item i, Mobile to, Layer layer )
		{
			if ( m_Pending == i.Serial )
			{
				...
				ClientCommunication.Instance.SendToServer( new EquipRequest( i.Serial, to, layer ) );
		...
		public static bool Drop( Item i, Serial dest, Point3D pt )
		{
			if ( m_Pending == i.Serial )
			{
				...
				ClientCommunication.Instance.SendToServer( new DropRequest( i.Serial, pt, dest ) );
				m_Pending = Serial.Zero;
				m_Lifted = DateTime.MinValue;
				return true;
			}
		...
		public static ProcStatus ProcessNext( int numPending )
		{
			if ( m_Pending != Serial.Zero )
			{
				if ( m_Lifted + TimeSpan.FromMinutes( 2 ) < DateTime.UtcNow )
				{
					...
					if ( World.Player != null )
					{
						...
						if ( World.Player.Backpack != null )
							ClientCommunication.Instance.SendToServer( new DropRequest( m_Pending, Point3D.MinusOne, World.Player.Backpack.Serial ) ); 
						else
							ClientCommunication.Instance.SendToServer( new DropRequest( m_Pending, World.Player.Position, Serial.Zero ) ); 
					}
			...
			if ( lr != null )
			{
				...
				ClientCommunication.Instance.SendToServer( new LiftRequest( lr.Serial, lr.Amount ) );
				...
				if ( dr != null )
				{
					...
					if ( dr.Serial.IsMobile && dr.Layer > Layer.Invalid && dr.Layer <= Layer.LastUserValid )
						ClientCommunication.Instance.SendToServer( new EquipRequest( lr.Serial, dr.Serial, dr.Layer ) );
					else 
						ClientCommunication.Instance.SendToServer( new DropRequest( lr.Serial, dr.Point, dr.Serial ) );
				}
				else
				{
					m_Pending = lr.Serial;
			...
			protected override void OnTick()
			{
				...
				if ( m_Queue != null && m_Queue.Count > 0 )
				{
					...
					while ( m_Queue.Count > 0 )
					{
						...
						{
							m_Queue.Dequeue();
							ClientCommunication.Instance.SendToServer( new DoubleClick( s ) );
							break;
						}
					}
