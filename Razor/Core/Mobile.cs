...
namespace Assistant
{
    ...
    public class Mobile : UOEntity
    {
        ...
        internal void OverheadMessageFrom(int hue, string from, string text, bool ascii)
        {
            if (ascii)
            {
                ClientCommunication.Instance.SendToClient(new AsciiMessage(Serial, m_Body, MessageType.Regular, hue, 3, from, text));
            }
            else
            {
                ClientCommunication.Instance.SendToClient(new UnicodeMessage(Serial, m_Body, MessageType.Regular, hue, 3, Language.CliLocName, from, text));
            }
        }
