...
namespace Assistant
{
	...
	public class SpecialMoves
	{
		...
		{
			ClientCommunication.Instance.SendToServer( new SetWarMode( !World.Player.Warmode ) );
		}
		...
		private static void OnStun()
		{
			if ( m_LastToggle+TimeSpan.FromSeconds( 0.5 ) < DateTime.UtcNow )
			{
				...
				ClientCommunication.Instance.SendToServer( new StunRequest() );
			}
		}
		...
		private static void OnDisarm()
		{
			...
			{
				m_LastToggle = DateTime.UtcNow;
				ClientCommunication.Instance.SendToServer( new DisarmRequest() );
			}
		}
        ...
        public static void SetPrimaryAbility()
		{
			...
			if ( a != AOSAbility.Invalid )
			{
				ClientCommunication.Instance.SendToServer( new UseAbility( a ) );
				ClientCommunication.Instance.SendToClient( ClearAbility.Instance );
		...
		{
			ClientCommunication.Instance.SendToServer( new UseAbility( AOSAbility.Clear ) );
			ClientCommunication.Instance.SendToClient( ClearAbility.Instance );
