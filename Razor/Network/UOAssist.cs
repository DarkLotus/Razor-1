		...
		public static int OnUOAMessage(MainForm razor, int Msg, int wParam, int lParam)
		{
			switch ((UOAMessage)Msg)
			{
				...
				case UOAMessage.DISPLAY_TEXT:
					{
						...
						if ((wParam & 0x00010000) != 0)
							ClientCommunication.Instance.SendToClient(new UnicodeMessage(0xFFFFFFFF, -1, MessageType.Regular, hue, 3, Language.CliLocName, "System", sb.ToString()));
						else
				...
				case UOAMessage.GET_UO_HWND:
					{
						return ClientCommunication.Instance.ClientWindow.ToInt32();
					}
				...
				case UOAMessage.SET_SKILL_LOCK:
					{
						if (World.Player == null || wParam < 0 || wParam > World.Player.Skills.Length || lParam < 0 || lParam >= 3)
							return 0;
						ClientCommunication.Instance.SendToServer(new SetSkillLock(wParam, (LockType)lParam));
						return 1;
					}
